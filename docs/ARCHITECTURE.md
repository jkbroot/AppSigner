...
| `MachOClassDump` | Reads Objective-C class and selector names directly from the `__objc_classname` / `__objc_methname` string sections of a slice, so it lists types on modern chained-fixups binaries without loading or pointer-chasing them; filters punctuation noise into meaningful names. |
| `MachOStrings` | Reads printable string literals from `__cstring`, and rewrites a whole literal in place across every slice — length-constrained (the replacement must fit), so file size and section offsets never change. |
| `MethodPatch` / `PatchGenerator` | Models a method-return override (class, selector, typed return value) and emits Objective-C that swizzles the method — instance first, then class — to return the fixed value via `imp_implementationWithBlock`. |
| `PatchDylibBuilder` | Compiles the generated hook source into an injectable iOS arm64 dynamic library (`@executable_path/Frameworks/…`) with the local toolchain; its compile arguments are a pure function so the recipe is testable without building. |
| `StringPatch` / `StringPatcher` | Models an in-place string override targeting a specific binary in the bundle, and applies a list of them to an unpacked `.app` before signing. |
| `OTAManifest` | Builds the `itms-services` `manifest.plist` (assets + metadata) for an over-the-air install. |
| `OTACertificate` | Generates a self-signed TLS certificate for a given IP via `openssl` and imports it as a `SecIdentity` for the server. |
| `OTAServer` | A local HTTPS server (`Network.framework`) that serves the landing page, certificate, manifest and IPA over TLS, streaming the IPA in chunks; resolves the Mac's LAN address. |
